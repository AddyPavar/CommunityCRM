...
use CommunityCRM\model\CommunityCRM\ConfigQuery;
use CommunityCRM\model\CommunityCRM\Version;
...
class Bootstrapper
{
    ...
    public static function init($sSERVERNAME, $dbPort, $sUSER, $sPASSWORD, $sDATABASE, $sRootPath, $bLockURL, array $URL): void
    {
        ...
        self::$bootStrapLogger->debug("Starting CommunityCRM");
        SystemURLs::checkAllowedURL($bLockURL, $URL);
        self::initMySQLI();
        self::initPropel();

        if (self::isDatabaseEmpty()) {
            self::installCommunityCRMSchema();
        }
        self::initSession();
    ...
    {
        self::$bootStrapLogger->debug("Checking for CommunityCRM Database tables");
    ...
    private static function installCommunityCRMSchema(): void
    {
        self::$bootStrapLogger->info("Installing CommunityCRM Schema");
        $connection = Propel::getConnection();
        $version = new Version();
        ...
        $version->save();
        self::$bootStrapLogger->info("Installed CommunityCRM Schema version: " . SystemService::getInstalledVersion());
    }
