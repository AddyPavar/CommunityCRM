...
namespace CommunityCRM\Utils;

use CommunityCRM\dto\CommunityCRMRelease;
use CommunityCRM\dto\SystemConfig;
use CommunityCRM\dto\SystemURLs;
use CommunityCRM\FileSystemUtils;
use Github\Client;

class CommunityCRMReleaseManager
{
    // todo: make these const variables private after deprecating PHP7.0 #4948
    public const GITHUB_USER_NAME = 'communitycrm';
    ...
    public static function getReleaseFromString(string $releaseString): CommunityCRMRelease
    {
        if (empty($_SESSION['CommunityCRMReleases'])) {
            // The CommunityCRM releases have not yet been populated.
            ...
            return new CommunityCRMRelease(@['name' => $releaseString]);
        } else {
            LoggerUtils::getAppLogger()->debug('Attempting to service query for release string ' . $releaseString . ' from GitHub release cache');
            $requestedRelease = array_values(array_filter($_SESSION['CommunityCRMReleases'], fn ($r): bool => $r->__toString() === $releaseString));
            if (count($requestedRelease) === 1 && $requestedRelease[0] instanceof CommunityCRMRelease) {
                ...
                throw new \Exception('Provided string matched more than one CommunityCRM Release: ' . \json_encode($requestedRelease, JSON_THROW_ON_ERROR));
            }
        }
    }

    /**
     * @return CommunityCRMRelease[]
     */
    private static function populateReleases(): array
    {
        ...
        try {
            $logger->debug("Querying GitHub '" . CommunityCRMReleaseManager::GITHUB_USER_NAME . '/' . CommunityCRMReleaseManager::GITHUB_REPOSITORY_NAME . "' for CommunityCRM Releases");
            $gitHubReleases = $client->api('repo')->releases()->all(CommunityCRMReleaseManager::GITHUB_USER_NAME, CommunityCRMReleaseManager::GITHUB_REPOSITORY_NAME);
            $logger->debug('Received ' . count($gitHubReleases) . ' CommunityCRM releases from GitHub');
            foreach ($gitHubReleases as $r) {
                $release = new CommunityCRMRelease($r);
            ...
            usort($eligibleReleases, fn (CommunityCRMRelease $a, CommunityCRMRelease $b): bool => $a->compareTo($b) < 0);

            $logger->debug('Found ' . count($eligibleReleases) . ' eligible CommunityCRM releases on GitHub');
        } catch (\Exception $ex) {
    ...
    {
        $_SESSION['CommunityCRMReleases'] = self::populateReleases();
    }

    public static function isReleaseCurrent(CommunityCRMRelease $Release): bool
    {
        if (empty($_SESSION['CommunityCRMReleases'])) {
            // The CommunityCRM releases have not yet been populated.
            // Since populating the release list can be an expensive operation
            ...
            $CurrentRelease = $_SESSION['CommunityCRMReleases'][0];
    ...
    private static function getHighestReleaseInArray(array $eligibleUpgradeTargetReleases)
    {
        if (count($eligibleUpgradeTargetReleases) > 0) {
            usort($eligibleUpgradeTargetReleases, fn (CommunityCRMRelease $a, CommunityCRMRelease $b): bool => $a->compareTo($b) < 0);
    ...
    private static function getReleaseNextPatch(array $rs, CommunityCRMRelease $currentRelease)
    {
        $eligibleUpgradeTargetReleases = array_values(array_filter($rs, function (CommunityCRMRelease $r) use ($currentRelease): bool {
    ...
    private static function getReleaseNextMinor(array $rs, CommunityCRMRelease $currentRelease)
    {
        $eligibleUpgradeTargetReleases = array_values(array_filter($rs, function (CommunityCRMRelease $r) use ($currentRelease): bool {
    ...
    private static function getReleaseNextMajor(array $rs, CommunityCRMRelease $currentRelease)
    {
        $eligibleUpgradeTargetReleases = array_values(array_filter($rs, function (CommunityCRMRelease $r) use ($currentRelease): bool {
    ...
    public static function getNextReleaseStep(CommunityCRMRelease $currentRelease): CommunityCRMRelease
    {
        LoggerUtils::getAppLogger()->debug('Determining the next-step release step for ' . $currentRelease);
        if (empty($_SESSION['CommunityCRMReleases'])) {
            $_SESSION['CommunityCRMReleases'] = self::populateReleases();
        }
        $rs = array_values($_SESSION['CommunityCRMReleases']);
        ...
        LoggerUtils::getAppLogger()->debug('Evaluating next-step release eligibility based on ' . count($_SESSION['CommunityCRMReleases']) . ' available releases ');
    ...
    public static function downloadLatestRelease(): array
    {
        ...
        $releaseToDownload = CommunityCRMReleaseManager::getNextReleaseStep(CommunityCRMReleaseManager::getReleaseFromString($_SESSION['sSoftwareInstalledVersion']));

        return CommunityCRMReleaseManager::downloadRelease($releaseToDownload);
    }

    public static function downloadRelease(CommunityCRMRelease $release): array
    {
    ...
    public static function preShutdown(): void
    {
        ...
        if (self::$isUpgradeInProgress) {
            ...
            echo \json_encode([
                'code'    => 500,
                'message' => 'Maximum execution time threshold exceeded: ' . ini_get('max_execution_time') . '.  This CommunityCRM installation may now be in an unstable state.  Please review the documentation at https://github.com/CommunityCRM/CRM/wiki/Recovering-from-a-failed-update',
            ], JSON_THROW_ON_ERROR);
        }
    }
    ...
    public static function doUpgrade(string $zipFilename, string $sha1): void
    {
        ...
        register_shutdown_function(fn () => CommunityCRMReleaseManager::preShutdown());
        ...
        if ($zip->open($zipFilename) === true) {
            ...
            FileSystemUtils::moveDir(SystemURLs::getDocumentRoot() . '/Upgrade/communitycrm', SystemURLs::getDocumentRoot());
