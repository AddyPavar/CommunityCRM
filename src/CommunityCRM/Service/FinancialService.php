...
class FinancialService
{
    ...
    private function generateBankDepositSlip($thisReport): void
    {
        ...
        $thisReport->pdf->Write(8, SystemConfig::getValue('sCommunityName'));

        $thisReport->pdf->SetXY($thisReport->AccountNumberX, $thisReport->AccountNumberY);
        $thisReport->pdf->Cell(55, 7, SystemConfig::getValue('sCommunityChkAcctNum'), 1, 1, 'R');
    ...
    public function getDepositCSV(string $depID): \stdClass
    {
        ...
        // Export file
        $CSVReturn->header = 'Content-Disposition: attachment; filename=CommunityCRM-DepositCSV-' . $depID . '-' . date(SystemConfig::getValue('sDateFilenameFormat')) . '.csv';
