...
SQL;
    ...
    public function getGroupStats(): array
    {
        ...
        (select count(*) from group_grp where grp_Type = 4 ) as EducationInitiativeClasses,
        (Select count(*) from person_per
          ...
          where fam_DateDeactivated is  null and
	            p2g2r_rle_ID = 2 and grp_Type = 4) as EducationInitiativeKidsCount
        from dual ;
        ';
        $rsQuickStat = RunQuery($sSQL);
        $row = mysqli_fetch_array($rsQuickStat);
        $data = ['groups' => $row['Group_cnt'], 'sundaySchoolClasses' => $row['EducationInitiativeClasses'], 'sundaySchoolkids' => $row['EducationInitiativeKidsCount']];
     ...
     * @param int $limit
     *
     * @return array|\CommunityCRM\Person[]|mixed|\Propel\Runtime\ActiveRecord\ActiveRecordInterface[]|\Propel\Runtime\Collection\ObjectCollection
     */
