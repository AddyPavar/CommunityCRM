...
$sPageTitle = gettext('Education Initiative Dashboard');
...
<div class="card">
  ...
  <div class="card-body">
    ...
    <a href="EducationInitiativeReports.php" class="btn btn-app"
       title="<?= gettext('Generate class lists and attendance sheets'); ?>"><i
        class="fa fa-file-pdf"></i><?= gettext('Reports'); ?></a>
    <a href="EducationInitiativeClassListExport.php" class="btn btn-app"
...
<div class="card card-info">
  <div class="card-header">
    <h3 class="card-title"><?= gettext('Education Initiative Classes') ?></h3>
  ...
  <div class="card-body">
    <table id="educationinitiativeMissing" class="table table-striped table-bordered data-table" cellspacing="0" width="100%">
      <thead>
      <tr>
        <th></th>
        <th><?= gettext('Class') ?></th>
      ...
      <?php foreach ($classStats as $class) {
        ...
        <tr>
          <td style="width:80px">
            <a href='EducationInitiativeClassView.php?groupId=<?= $class['id'] ?>'>
...
<div class="card card-danger">
  <div class="card-header">
    <h3 class="card-title"><?= gettext('Students not in a Education Initiative Class') ?></h3>
      <div class="card-tools pull-right">
          <button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fa fa-minus"></i>
          </button>
          <button type="button" class="btn btn-tool" data-card-widget="remove"><i class="fa fa-times"></i>
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title"
              id="delete-Image-label"><?= gettext('Add') ?> <?= gettext('Education Initiative') ?> <?= gettext('Class') ?> </h4>
        </div>
      ...
      $("#addNewClassBtn").click(function (e) {
        ...
        if (groupName) // ensure that the user entered a name
        {
          $.ajax({
            ...
            data: JSON.stringify({
              'groupName': groupName,
              'isEducationInitiative': true
            })
          }).done(function (data) {                               //yippie, we got something good back from the server
            window.location.href = window.CRM.root + "/educationinitiative/EducationInitiativeClassView.php?groupId=" + data.Id;
          });
        }
        else {
